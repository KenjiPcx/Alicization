        ...
        if (direction === 'vertical') {
            if (position.includes('top')) {
                ...
                if (position.includes('right')) {
                    return `${baseClasses} top-16 right-1 space-y-3`; // Offset to center with trigger
                } else {
                    return `${baseClasses} top-16 left-1 space-y-3`; // Offset to center with trigger
                }
            } else {
                // For bottom positions, center items relative to trigger button
                if (position.includes('right')) {
                    return `${baseClasses} bottom-16 right-1 space-y-3`; // Offset to center with trigger
                } else {
                    return `${baseClasses} bottom-16 left-1 space-y-3`; // Offset to center with trigger
                }
            }
        } else {
            if (position.includes('left')) {
                return `${baseClasses} left-16 space-x-3 flex`;
            } else {
                return `${baseClasses} right-16 space-x-3 flex`;
            }
        }
    };
        ...
        <div className={cn(positioning, "z-50", positionClasses[position], className)}>
            <div className="relative">
                ...
                >
                    ...
                    >
                        {isOpen ? <X className="h-6 w-6" /> : <TriggerIcon className="h-6 w-6" />}
                    </Button>
                </motion.div>
