...
export const chatWorkflow = workflow.define({
    ...
    handler: async (step, args): Promise<{ status: string; message: string; todoProcessingRound?: number }> => {
        return { status: "completed", message: "Chat workflow completed" };
    },
    // handler: async (step, args): Promise<{ status: string; message: string; todoProcessingRound?: number }> => {
    ...
    // await step.runAction(internal.node.chat.streamMessage, {
    //     threadId,
    //     userId,
    //     employeeId,
    //     teamId,
...
export const cleanupWorkflowHelper = async (ctx: MutationCtx, workflowId: WorkflowId) => {
    try {
        while (true) {
