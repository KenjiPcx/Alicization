...
import { getAuthUserId, patchCompany } from "@/convex/utils";
...
export const getCompany = query({
    ...
    handler: async (ctx, { fetchTeams, fetchEmployees }): Promise<CompanyData> => {
        ...
        return await ctx.runQuery(internal.office.companies._getCompany, {
            userId,
            fetchTeams,
            fetchEmployees
        });
...
export const _getCompany = internalQuery({
    args: v.object({
        userId: v.string(),
        fetchTeams: v.optional(v.boolean()),
        fetchEmployees: v.optional(v.boolean()),
 ...
 */
export const _updateCompany = internalMutation({
    args: v.object({
        userId: v.string(),
        name: v.optional(v.string()),
        vision: v.optional(v.string()),
